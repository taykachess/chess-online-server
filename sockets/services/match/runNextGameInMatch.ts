...
export async function runNextGameInMatch({ matchId, match }: { matchId: string; match: Match }) {
  ...
  if (status == 'running') {
    ...
    Promise.all([redis.SADD(PLAYER_IN_GAME_REDIS(match.player1), gameId), redis.SADD(PLAYER_IN_GAME_REDIS(match.player2), gameId)])
  }
