...
export async function finishTournamentGame({ game, gameId, tournamentId, result }: { game: Game; gameId: string; tournamentId: string; result: Result }) {
  ...
  if (currentActiveGames == 0) {
    ...
    if (round > maxRound[0]) {
      ...
      await setTournamentActiveGames(tournamentId, activePlayers.length % 2 == 0 ? pairings.length : pairings.length - 1)
    }
  }
}
