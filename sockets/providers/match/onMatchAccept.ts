...
import { MATCHES_REDIS, PLAYER_IN_GAME_REDIS } from '../../variables/redisIndex'

import type { GetMatch } from '../../types/match'
import type { SocketType } from '../../types/sockets'
import { createMatch } from '../../services/match/createMatch'

export async function onMatchAccept(this: SocketType, { username }: { username: string }) {
  const socket = this
  ...
  try {
    ...
    if (match.socketId == socket.id) throw Error("You can't accept your own match")
