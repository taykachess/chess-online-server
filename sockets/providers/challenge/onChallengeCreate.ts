...
import { CHALLENGES_REDIS, CHALLENGES_ROOM, PLAYER_IN_GAME_REDIS } from '../../variables/redisIndex'

import type { ChallengeFilters, GetChallenge } from '../../types/challenge'
import type { SocketType } from '../../types/sockets'

export async function onChallengeCreate(this: SocketType, data: { control: string; filters: ChallengeFilters }) {
  try {
    const socket = this
    ...
    if (existChallenges?.length && existChallenges[0].user != socket.data.username) {
      const choosenChallenge = existChallenges[0]
      const [socket2] = await io.in(`${choosenChallenge.socketId}`).fetchSockets()
