...
export async function addGame(matchId: string, game: MatchGame, match: Match): Promise<Match> {
  ...
  await Promise.all([redis.json.ARRAPPEND(MATCHES_IN_PROGRESS_REDIS, `$.${matchId}.games`, game), redis.json.numIncrBy(MATCHES_IN_PROGRESS_REDIS, `$.${matchId}.result[${resultIndex}]`, 1)])
