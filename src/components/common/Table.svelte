            ...
            <thead class="bg-gray-50">
              <tr>
                {#each titles as title}
                  <th scope="col" class="whitespace-nowrap py-3.5 pl-4 pr-3  text-sm font-semibold text-gray-900 sm:pl-6">{title}</th>
                {/each}
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 bg-white  ">
              {#each records as record}
                <tr
                  ...
                  class="cursor-pointer hover:bg-slate-50 {record.registered ? ' bg-green-100' : ''}"
                >
                  {#each record.records as field}
                    <td class="whitespace-nowrap py-2 pl-4 pr-3 text-sm font-medium  text-gray-900 sm:pl-6">{field}</td>
                  {/each}
                </tr>
              {/each}
