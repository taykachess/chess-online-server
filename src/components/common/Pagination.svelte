...
<div class=" {textClass ? textClass : 'text-gray-800'} flex    flex-1 select-none items-center justify-between ">
  <div>
    <p class="text-sm ">
      Показаны с
      <span class="font-medium">{(currentPage - 1) * STEP + 1}</span>
      по
      <span class="font-medium">{currentPage * STEP < count ? currentPage * STEP : count}</span>
      от
  ...
  <nav class=" flex rounded-md border  shadow-sm" aria-label="Pagination">
    ...
    {#each arrayPages(PAGES) as page, index}
      ...
      <div
        ...
        aria-current="page"
        class="relative {currentPage == page || currentPage + 1 == page || currentPage - 1 == page || page == 1 || page == PAGES ? ' ' : 'hidden'}  cursor-pointer items-center  {page === currentPage
