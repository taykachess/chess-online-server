...
>
  <ListboxButton
    ...
    >{@html selectedOption?.name}</ListboxButton
  >
  <ListboxOptions
    class="absolute top-0 z-10  rounded  {colorOptions.bg}  text-xs"
  >
    {#each options as option (option.value)}
      <ListboxOption
        class="flex w-40 cursor-pointer items-center justify-between rounded px-2 py-0.5 {colorOptions.hover}"
        selected
        value={option}
        let:selected
      >
