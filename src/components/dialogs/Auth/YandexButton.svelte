<script lang="ts">
  ...
  const yandexRedirect = type == 'login' ? `${dev ? PUBLIC_APP_DEV_URL : PUBLIC_APP_PROD_URL}/api/auth/yandex/login` : `${dev ? PUBLIC_APP_DEV_URL : PUBLIC_APP_PROD_URL}/api/auth/yandex/signup`
</script>
...
>
  <div class=" flex h-5 w-5 items-center justify-center rounded-full bg-rose-500 ">Я</div>
