...
{#if $info?.tree?.history}
  <div class=" grid w-60 grid-cols-10    overflow-hidden rounded-l-lg border-x border-slate-600 text-center text-slate-700">
    {#each $info.tree?.history as move}
      ...
      <div
        ...
        class="relative col-span-4 cursor-pointer {move.ply % 2 != 0 ? 'border-r border-slate-600' : ' '}  hover:font-bold hover:text-blue-800 {$info.tree.currentNode == move
