<script lang="ts">
  ...
  console.log(periods)
  $: timeToNextStage = new Date(timestamp).getTime() + periods[stage - 1][0] * 60 * 1000
</script>
...
<div class=" mr-2 flex flex-col items-center justify-center ">
  ...
  {#each periods as period, index}
    <div class=" grid w-full grid-cols-7 divide-y border-r font-medium {periods.length - 1 == index ? ' border-b' : ''}  text-center  text-slate-800 ">
