<script lang="ts">
  ...
  async function getInitialMatches() {
    const [matchesData, countData] = await Promise.all([getAllMatches(), getCountAllMatches()])
  ...
  function createMatchesRecords(match: GetMatch[] | null): ChallengeTableRecord[] {
    ...
    match.forEach((match) => {
      arrayRecords.push({
        ...
        records: [match.user, `${match.rating}`, match.control, `${match.rounds}`],
      })
    })
  ...
  onMount(async () => {
    ...
    $socket.on('match:created', (match) => {
      const index = $listOfMatches.matches.findIndex((chal) => chal.user == match.user)
      if (index === -1) {
        ...
        const index2 = records.findIndex((record) => record.records[0] == $listOfMatches.matches[index]?.user)
        records[index2] = {
          onClick: () => {
            $socket.emit('match:accept', { username: match.user })
          },
    ...
    $socket.on('match:deleted', ({ socketId }) => {
      const index = $listOfMatches.matches.findIndex((chal) => chal.socketId == socketId)
      if (index !== -1) {
        const index2 = records.findIndex((record) => record.records[0] == $listOfMatches.matches[index]?.user)
        if (index2 !== -1) {
