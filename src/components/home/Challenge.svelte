<script lang="ts">
  ...
  async function getInitialChallenges() {
    const [challengesData, countData] = await Promise.all([getAllChallenges(), getCountAllChallenges()])
  ...
  function createChallengeRecords(challenge: GetChallenge[] | null): ChallengeTableRecord[] {
  ...
  onMount(async () => {
    ...
    $socket.on('challenge:created', (challenge) => {
      const index = $listOfChallenges.challenges.findIndex((chal) => chal.user == challenge.user)
      if (index === -1) {
        ...
        const index2 = records.findIndex((record) => record.records[0] == $listOfChallenges.challenges[index]?.user)
        records[index2] = {
          onClick: () => {
            $socket.emit('challenge:accept', { username: challenge.user })
          },
    ...
    $socket.on('challenge:deleted', ({ socketId }) => {
      const index = $listOfChallenges.challenges.findIndex((chal) => chal.socketId == socketId)
      if (index !== -1) {
        const index2 = records.findIndex((record) => record.records[0] == $listOfChallenges.challenges[index]?.user)
        if (index2 !== -1) {
