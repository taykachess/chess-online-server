...
<div class="inline-block transform overflow-hidden  rounded-lg bg-white text-left align-middle  shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg ">
  <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 ">
    ...
    <div class="flex flex-wrap divide-y divide-slate-200 border-b border-slate-200 text-sm dark:divide-slate-200/5 dark:border-slate-200/5 sm:text-base lg:text-sm xl:text-base">
      <div class=" px-4  pb-4 sm:px-6 lg:px-4 xl:px-6">
        <DateInput bind:value={formData.startTime} format="yyyy-MM-dd HH:mm" min={new Date()} max={new Date(new Date().getTime() + 60 * 60 * 1000 * 24 * 31)} placeholder="время и дата" />
      </div>
      ...
      <div class="flex w-full flex-none  items-center p-4 sm:p-6 lg:p-4 xl:p-6">
        <dt class="w-2/5 flex-none font-medium text-slate-500  transition-opacity delay-500 duration-[1.5s] dark:text-slate-300 sm:w-1/4">Описание</dt>
        <textarea rows="2" class="  h-full w-full border-0 bg-transparent text-sm placeholder:text-slate-200 focus:ring-0" placeholder="The best tournament " />
      </div>
    </div>
  </div>
  <div class="grid grid-cols-2 gap-x-4 p-4 sm:gap-x-6 sm:px-6 sm:py-5 lg:gap-x-4 lg:p-4 xl:gap-x-6 xl:px-6 xl:py-5">
    <button
    ...
    <button on:click={createTournament} class="dark:highlight-white/20 cursor-pointer rounded-lg bg-sky-500 py-3 text-center text-base font-medium text-white"> Создать турнир </button>
  </div>
</div>
