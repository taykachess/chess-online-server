...
<div class=" relative z-50  ">
  ...
  {#if isOpen}
    <div use:attachWindowEvent on:mousedown|stopPropagation={() => {}} class=" absolute right-2 mt-1 w-[30rem] select-none border bg-white shadow-xl ">
      {#if $privateMatches.length}
        <ul class=" flex  max-h-96  scroll-py-3 flex-col  overflow-y-auto  " id="options" role="listbox">
          {#each $privateMatches as match, index}
            <li class="group flex cursor-default  select-none items-center   {index % 2 == 0 ? ' bg-white' : ''}  p-2 ">
              <div class=" flex  w-full items-center justify-between ">
                ...
                <div class=" flex">
                  ...
                  <div class=" h-8 w-8 cursor-pointer text-red-600 hover:scale-105 hover:text-red-700">
                    <IconXCircle />
                  </div>
                </div>
              </div>
