...
export const POST: RequestHandler = async ({ locals, request }) => {
  ...
  const dataMatch = await redis.hGet(MATCHES_REDIS_GOT(locals.user.username), `${player}`)
  ...
  const status = await redis.hDel(MATCHES_REDIS_GOT(locals.user.username), `${player}`)
  ...
  if (status == 1) emitter.to(USER_ROOM(player)).emit('match:private:accepted', matchFromPrisma.id)
