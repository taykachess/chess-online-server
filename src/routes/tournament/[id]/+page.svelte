<script lang="ts">
  ...
  if (browser) {
    if (data.liveGameId) {
      $socket.emit('game:get', { gameId: data.liveGameId }, (gameFromServer) => {
  ...
  function setTournamentTv(game: GetGame, gameId: string) {
    if ($tournamentTv && $tournamentTv.game && $tournamentTv.game.id && $tournamentTv.game.result == '*') $socket.emit('game:leave', { gameId: $tournamentTv.game.id })
    ...
    if (game.result == '*') {
      if (!game.pgn) {
        $tournamentPrepareTime = game.tsmp + TOURNAMENT_GAME_PREPARE_TIME - new Date().getTime()
  ...
  onMount(async () => {
    ...
    $socket.on('tournament:unregister', ({ username }) => {
      const index = $tournament.participants.findIndex((participant) => participant.username === username)
    ...
    $socket.on('tournament:pause', ({ username }) => {
      const indexPausePlayer = $tournament.players.findIndex((player) => player.id == username)
      if (indexPausePlayer != -1) $tournament.players[indexPausePlayer].active = false
    })

    $socket.on('tournament:continue', ({ username }) => {
      const indexPausePlayer = $tournament.players.findIndex((player) => player.id == username)
      if (indexPausePlayer != -1) $tournament.players[indexPausePlayer].active = true
    })
  ...
  beforeNavigate(() => {
    ...
    if ($tournament.status == 'running') $socket.emit('game:leave', { gameId: $selectedTournamentGameId })
  })
</script>
...
<div class=" mx-5 max-w-6xl sm:mx-auto">
  ...
  <div class="flex flex-col-reverse sm:sticky sm:top-0 sm:mt-8 sm:grid sm:grid-cols-5 sm:space-x-8">
