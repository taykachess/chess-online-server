...
export const load: PageServerLoad = async ({ params, fetch }) => {
  ...
  if (tournamentWithStatus.status == 'running') {
    ...
    const [currentRound] = (await redis.json.get(TOURNAMENTS_IN_PROGRESS_REDIS, {
      path: `$.${params.id}.round`,
    })) as [number]
    const currentRoundValue = currentRound > tournament.rounds ? tournament.rounds : currentRound

    const data = await fetch(`/api/tournament/${params.id}/pairings?round=${currentRoundValue - 1}`)

    const matches = await data.json()

    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    const [players]: [Record<string, PlayerSwiss>] = await redis.json.get(TOURNAMENTS_IN_PROGRESS_REDIS, {
